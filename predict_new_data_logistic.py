...
def predict_new_data(data_file, ID_column_list):
  ...
  bbeta_data = pickle.load(open('bbeta_dict.pkl','rb'))
  beta_dict = bbeta_data['beta']
  ...
  prediction = np.divide(np.exp(logistic_input), (1+np.exp(logistic_input)))
  ID_cols = data['ID_cols']
  ID_cols['prediction'] = pd.DataFrame(prediction)
  ID_cols.to_csv('test_prediction.csv', index=False)
